...
public class EnemyManager : Photon.PunBehaviour, IPunObservable
{
    ...
    void Awake()
    {
        ...
        //{
        //    Debug.Log("aaa");
        //   EnemyManager.LocalPlayerInstance = this.gameObject;
        //}
    }
    #endregion
    #region 頭上UIの生成
    void Start()
    {
        MoveEnemy = GetComponent<Move>();

        if (EnemyUiPrefab != null) //PlayerUIPrefabが設定されていない時のエラー回避
        {
            //Playerの頭上UIの生成とPlayerUIScriptでのSetTarget関数呼出
            _uiGo = Instantiate(EnemyUiPrefab) as GameObject;
    ...
    void Update()
    {
        ...
        else
        {
            ...
            //if (!photonView.isMine|| uiActive == false)
            //{
            //    return;
            //}
